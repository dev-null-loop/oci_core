resource "oci_core_vnic_attachment" "this" {
  create_vnic_details {
    ...
    dynamic "ipv6address_ipv6subnet_cidr_pair_details" {
      for_each = var.create_vnic_details.ipv6address_ipv6subnet_cidr_pair_details
      iterator = iicpd
      content {
    ...
    private_ip_id          = var.create_vnic_details.private_ip_id
    ...
    subnet_cidr            = var.create_vnic_details.subnet_cidr
    subnet_id              = var.create_vnic_details.subnet_id
    vlan_id                = var.create_vnic_details.vlan_id
    route_table_id         = var.create_vnic_details.route_table_id
  }
